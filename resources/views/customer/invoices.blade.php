                            ...
                            @forelse($invoices as $invoice)
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-6 py-4 text-sm text-gray-600">
                                        {{ $invoice->period ?? $invoice->created_at->format('F Y') }}</td>
                                    ...
                                    <td class="px-6 py-4">
                                        @if($invoice->status == 'paid')
                                            <span
                                                class="inline-flex px-2 py-1 text-xs rounded-full bg-green-100 text-green-700">Lunas</span>
                                        @elseif($invoice->due_date < now())
                                            <span class="inline-flex px-2 py-1 text-xs rounded-full bg-red-100 text-red-700">Jatuh
                                                Tempo</span>
                                        @else
                                            <span class="inline-flex px-2 py-1 text-xs rounded-full bg-yellow-100 text-yellow-700">Belum
                                                Bayar</span>
                                        @endif
                                    </td>
                                    <td class="px-6 py-4">
                                        @if($invoice->status == 'unpaid')
                                                     <a href="{{ route('customer.pay', $invoice) }}" target="_blank" class="bg-cyan-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-cyan-700">
