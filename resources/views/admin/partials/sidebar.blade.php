...
<style>
            ...
            .nc-link i { width: 1.1rem; text-align: center; color: var(--noc-text-dim); transition: all .3s cubic-bezier(.4,0,.2,1); }
            .nc-link:hover {
            ...
            .nc-link:hover i { color: var(--noc-cyan); transform: scale(1.12); }
            ...
            .nc-link-active i { color: var(--noc-cyan); }

            /* ---------- submenu links ---------- */
            .nc-submenu { position: relative; }
            .nc-submenu::before {
                content: '';
                position: absolute;
            ...
            .nc-chevron { transition: transform .3s cubic-bezier(.4,0,.2,1); color: var(--noc-text-dim); }
            .nc-chevron-open { transform: rotate(180deg); color: var(--noc-cyan); }

            .nc-spin { animation: nc-spin-once .5s cubic-bezier(.4,0,.2,1); }
            ...
            @keyframes nc-pulse {
                ...
                50% { box-shadow: 0 0 15px 0px rgba(239, 68, 68, 0.85); }
            }

            /* ---------- bottom actions ---------- */
            .nc-btn { transition: all .3s cubic-bezier(.4,0,.2,1); }
            ...
            /* ---------- scrollbar ---------- */
            .noc-sidebar nav::-webkit-scrollbar { width: 4px; }
            .noc-sidebar nav::-webkit-scrollbar-thumb { background: rgba(34, 211, 238, 0.25); border-radius: 4px; }
            .noc-sidebar nav::-webkit-scrollbar-track { background: transparent; }
</style>

<div class="noc-sidebar fixed inset-y-0 left-0 z-50 w-64 transform transition-transform duration-300 ease-in-out lg:translate-x-0"
    ...
    <div class="relative z-10 flex flex-col h-full">
        ...
        <nav class="mt-4 px-2 space-y-1 overflow-y-auto flex-1" style="max-height: calc(100vh - 180px);">
            ...
            <a href="{{ route('admin.dashboard') }}"
                class="nc-link {{ request()->routeIs('admin.dashboard') ? 'nc-link-active' : '' }}">
                <i class="fas fa-home"></i>
                <span>Dashboard</span>
            </a>

            <div x-data="{ openCustomers: {{ request()->routeIs('admin.customers.*') ? 'true' : 'false' }}, spinCustomers: false }">
                <button @click="openCustomers = !openCustomers; spinCustomers = true; setTimeout(() => spinCustomers = false, 500)"
                    class="nc-link w-full {{ request()->routeIs('admin.customers.*') ? 'nc-link-active' : '' }}">
                ...
                <div x-show="openCustomers" x-cloak
                    ...
                    class="nc-submenu mt-1 ml-4 pl-4 space-y-1 origin-top">
            ...
            <a href="{{ route('admin.packages.index') }}"
                class="nc-link {{ request()->routeIs('admin.packages.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-box"></i>
                <span>Packages</span>
            </a>

            <a href="{{ route('admin.invoices.index') }}"
                class="nc-link {{ request()->routeIs('admin.invoices.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-file-invoice"></i>
                <span>Invoices</span>
            </a>

            <a href="{{ route('admin.orders.index') }}"
                class="nc-link {{ request()->routeIs('admin.orders.*') ? 'nc-link-active' : '' }}">
            ...
            <a href="{{ route('admin.expenses.index') }}"
                class="nc-link {{ request()->routeIs('admin.expenses.*') ? 'nc-link-active' : '' }}">
            ...
            <a href="{{ route('admin.technicians.index') }}"
                class="nc-link {{ request()->routeIs('admin.technicians.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-tools"></i>
                <span>Technicians</span>
            </a>

            <a href="{{ route('admin.collectors.index') }}"
                class="nc-link {{ request()->routeIs('admin.collectors.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-hand-holding-usd"></i>
                <span>Collectors</span>
            </a>

            <a href="{{ route('admin.attendance.index') }}"
                class="nc-link {{ request()->routeIs('admin.attendance.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-clipboard-check"></i>
                <span>Absensi</span>
            </a>

            <a href="{{ route('admin.attendance.history') }}"
                class="nc-link {{ request()->routeIs('admin.attendance.history') ? 'nc-link-active' : '' }}">
            ...
            <a href="{{ route('admin.agents.index') }}"
                class="nc-link {{ request()->routeIs('admin.agents.*') ? 'nc-link-active' : '' }}">
            ...
            <a href="{{ route('admin.olt.index') }}"
                class="nc-link {{ request()->routeIs('admin.olt.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-broadcast-tower"></i>
                <span>OLT Management</span>
            </a>

            <a href="{{ route('admin.vouchers.index') }}"
                class="nc-link {{ request()->routeIs('admin.vouchers.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-ticket-alt"></i>
                <span>Vouchers</span>
            </a>

            <a href="{{ route('admin.network.odps.index') }}"
                class="nc-link {{ request()->routeIs('admin.network.odps.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-project-diagram"></i>
                <span>ODP Management</span>
            </a>

            <a href="{{ route('admin.network.map') }}"
                class="nc-link {{ request()->routeIs('admin.network.map') ? 'nc-link-active' : '' }}">
                <i class="fas fa-map-marked-alt"></i>
                <span>Network Map</span>
            </a>

            <a href="{{ route('admin.network.tiangs.index') }}"
                class="nc-link {{ request()->routeIs('admin.network.tiangs.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-heading"></i>
                <span>Tiang Management</span>
            </a>

            <div x-data="{ openModems: {{ request()->routeIs('admin.network.modems.*') ? 'true' : 'false' }}, spinModems: false }">
                <button @click="openModems = !openModems; spinModems = true; setTimeout(() => spinModems = false, 500)"
                    class="nc-link w-full {{ request()->routeIs('admin.network.modems.*') ? 'nc-link-active' : '' }}">
            ...
            <a href="{{ route('admin.network.backbones.index') }}"
                class="nc-link {{ request()->routeIs('admin.network.backbones.*') ? 'nc-link-active' : '' }}">
            ...
            <a href="{{ route('admin.mikrotik.index') }}"
                class="nc-link {{ request()->routeIs('admin.mikrotik.index') ? 'nc-link-active' : '' }}">
                <i class="fas fa-server"></i>
                <span>Mikrotik</span>
            </a>

            <a href="{{ route('admin.mikrotik.sync.index') }}"
                class="nc-link {{ request()->routeIs('admin.mikrotik.sync.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-sync-alt"></i>
                <span>Mikrotik Sync</span>
            </a>

            <a href="{{ route('admin.radius.index') }}"
                class="nc-link {{ request()->routeIs('admin.radius.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-shield-alt"></i>
                <span>RADIUS</span>
            </a>

            <a href="{{ route('admin.cpe.index') }}"
                class="nc-link {{ request()->routeIs('admin.cpe.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-wifi"></i>
                <span>CPE / ONU</span>
            </a>

            <a href="{{ route('admin.snmp.index') }}"
                class="nc-link {{ request()->routeIs('admin.snmp.*') ? 'nc-link-active' : '' }}">
                <i class="fas fa-chart-line"></i>
                <span>SNMP Monitor</span>
            </a>

            <a href="{{ route('admin.ip-monitor.index') }}"
                class="nc-link {{ request()->routeIs('admin.ip-monitor.*') ? 'nc-link-active' : '' }}">
            ...
            <a href="{{ route('admin.whatsapp.index') }}"
                class="nc-link {{ request()->routeIs('admin.whatsapp.*') ? 'nc-link-active' : '' }}">
                <i class="fab fa-whatsapp"></i>
                <span>WhatsApp</span>
            </a>

            <a href="{{ route('admin.payment.index') }}"
                class="nc-link {{ request()->routeIs('admin.payment.*') ? 'nc-link-active' : '' }}">
            ...
            <a href="{{ route('admin.reports.index') }}"
                class="nc-link {{ request()->routeIs('admin.reports.index') ? 'nc-link-active' : '' }}">
                <i class="fas fa-chart-bar"></i>
                <span>Overview</span>
            </a>

            <a href="{{ route('admin.reports.daily') }}"
                class="nc-link {{ request()->routeIs('admin.reports.daily') ? 'nc-link-active' : '' }}">
                <i class="fas fa-calendar-day"></i>
                <span>Daily Report</span>
            </a>

            <a href="{{ route('admin.reports.monthly') }}"
                class="nc-link {{ request()->routeIs('admin.reports.monthly') ? 'nc-link-active' : '' }}">
            ...
            <div x-data="{ openTickets: {{ request()->routeIs('admin.ticket_gangguan.*') || request()->routeIs('admin.tickets.*') ? 'true' : 'false' }}, spinTickets: false }">
                <button @click="openTickets = !openTickets; spinTickets = true; setTimeout(() => spinTickets = false, 500)"
                    class="nc-link w-full {{ request()->routeIs('admin.ticket_gangguan.*') || request()->routeIs('admin.tickets.*') ? 'nc-link-active' : '' }}">
                ...
                <div x-show="openTickets" x-cloak
                    x-transition:enter="transition ease-out duration-300"
                    x-transition:enter-start="opacity-0 -translate-y-2 scale-95"
                    x-transition:enter-end="opacity-100 translate-y-0 scale-100"
                    x-transition:leave="transition ease-in duration-200"
                    ...
                    <a href="{{ route('admin.ticket_gangguan.index') }}"
                        class="nc-sublink {{ request()->routeIs('admin.ticket_gangguan.*') ? 'nc-sublink-active' : '' }}">Tiket Gangguan</a>
                    <a href="{{ route('admin.tickets.index') }}"
                        class="nc-sublink {{ request()->routeIs('admin.tickets.*') ? 'nc-sublink-active' : '' }}">Tiket Umum</a>
                </div>
            </div>
        ...
        <div class="p-4 bg-gradient-to-t from-black/40 to-transparent border-t border-cyan-500/10 backdrop-blur-sm">
            <div class="flex space-x-2">
                <a href="{{ route('admin.change-password') }}"
                    class="nc-btn nc-btn-password flex-1 flex items-center justify-center px-3 py-2 text-gray-300 rounded-lg text-sm">
                    <i class="fas fa-key mr-2"></i>Password
                </a>
                <form action="{{ route('admin.logout') }}" method="POST" class="flex-1">
                    @csrf
                    <button type="submit"
                        class="nc-btn nc-btn-logout w-full flex items-center justify-center px-3 py-2 text-gray-300 rounded-lg text-sm">
