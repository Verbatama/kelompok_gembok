            ...
            <div class="p-6">
                ...
                <div class="max-w-3xl">
                    <form action="{{ route('admin.technicians.update', $technician) }}" method="POST"
                        ...
                        <div class="space-y-6">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Name <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="name" value="{{ old('name', $technician->name) }}" required
                                    class="w-full px-4 py-2 border border-gray-300 dark:border-slate-500 dark:bg-slate-600 dark:text-white rounded-lg focus:ring-2 focus:ring-cyan-500">
                            ...
                            <div>
                                ...
                                <input type="number" name="gaji_pokok"
                                    value="{{ old('gaji_pokok', $technician->gaji_pokok ?? 0) }}" min="0"
                                    class="w-full px-4 py-2 border border-gray-300 dark:border-slate-500 dark:bg-slate-600 dark:text-white rounded-lg focus:ring-2 focus:ring-cyan-500"
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select name="is_active"
                                        class="w-full px-4 py-2 border border-gray-300 dark:border-slate-500 dark:bg-slate-600 dark:text-white rounded-lg focus:ring-2 focus:ring-cyan-500">
                                        <option value="1" {{ old('is_active', $technician->is_active) ? 'selected' : '' }}>
                                            Active</option>
                                        <option value="0" {{ !old('is_active', $technician->is_active) ? 'selected' : '' }}>
                                            Inactive</option>
                                    </select>
                                </div>
                            </div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Area
                                    Coverage</label>
                                <input type="text" name="area_coverage"
                                    value="{{ old('area_coverage', $technician->area_coverage) }}"
                                    class="w-full px-4 py-2 border border-gray-300 dark:border-slate-500 dark:bg-slate-600 dark:text-white rounded-lg focus:ring-2 focus:ring-cyan-500"
                                    placeholder="e.g., Jakarta Selatan, Depok">
                            </div>
                        </div>

                        <div
                            class="flex items-center justify-end space-x-3 mt-8 pt-6 border-t border-gray-200 dark:border-slate-600">
                            <a href="{{ route('admin.technicians.show', $technician) }}"
                                class="px-6 py-2 border border-gray-300 dark:border-slate-500 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-600 transition">Cancel</a>
                            <button type="submit"
                                class="bg-gradient-to-r from-cyan-500 to-blue-600 text-white px-6 py-2 rounded-lg hover:shadow-lg transition">
